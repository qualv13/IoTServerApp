...
public class SmartFeaturesScheduler {
    ...
    public void runSmartLogic() {
        List<Lamp> activeLamps = lampRepository.findAll().stream()
                ...
                .filter(Lamp::isOn)     // Tylko włączone (nie chcemy włączać lampy w nocy samoczynnie)
                .toList();

        for (Lamp lamp : activeLamps) {
            boolean changed = false;

            // 1. CIRCADIAN RHYTHM (Rytm Dobowy)
            if (lamp.isCircadianEnabled()) {
                applyCircadianRhythm(lamp);
                changed = true;
            }

            // 2. ADAPTIVE BRIGHTNESS (Jasność adaptacyjna)
            if (lamp.isAdaptiveBrightnessEnabled() && lamp.getLastAmbientLight() != null) {
                applyAdaptiveBrightness(lamp);
                changed = true;
            }

            // Jeśli coś zmieniliśmy w stanie lampy, zapisz to w bazie (ale nie wysyłamy tu komendy,
            // komendy wysyłają metody apply...)
            if(changed) {
    ...
    private void applyCircadianRhythm(Lamp lamp) {
        ...
        // Logika Rytmu Dobowego
        if (hour >= 6 && hour < 9) {
            // RANO: Budzenie (Mieszane)
            targetWarm = 100; targetCold = 150;
        } else if (hour >= 9 && hour < 17) {
            // PRACA: Zimne światło (Koncentracja)
            targetWarm = 0; targetCold = 255;
        } else if (hour >= 17 && hour < 20) {
            // POPOŁUDNIE: Neutralne
            targetWarm = 150; targetCold = 100;
        } else {
            // WIECZÓR/NOC: Ciepłe (Relaks, Melatonina)
            targetWarm = 255; targetCold = 0;
        }

        // Sprawdź czy trzeba zmienić (żeby nie spamować MQTT)
        // Zakładam, że w encji Lamp masz pola warmWhite/coldWhite. Jeśli nie, dodaj je.
        // Tutaj dla uproszczenia wysyłam komendę zawsze, w produkcji sprawdź różnicę > 10.
    ...
    private void applyAdaptiveBrightness(Lamp lamp) {
        ...
        // Algorytm odwrotny: Im jaśniej w pokoju, tym mniej światła z lampy
        // Próg celowy: Chcemy utrzymać np. poziom "średni".

        if (lux > 800) {
            // Bardzo jasno (słońce) -> Zmniejsz do minimum
            targetBrightness = 10;
        } else if (lux > 400) {
            // Jasno -> Średnia jasność
            targetBrightness = 40;
        } else if (lux < 100) {
            // Ciemno -> Pełna moc
        ...
        // Histereza (zmieniaj tylko jeśli różnica > 5%)
        if (Math.abs(targetBrightness - currentBrightness) > 5) {
            ...
            // Tutaj musielibyśmy zaktualizować jasność wszystkich kanałów proporcjonalnie
            // Dla uproszczenia, zakładamy, że to steruje ogólnym ściemniaczem (jeśli masz taki w sofcie)
            // lub po prostu aktualizujemy pole w bazie.

            lamp.setBrightness(targetBrightness);
            // W idealnym świecie wysyłamy tu komendę przeskalowania RGB/White
        }
    }
    ...
    private void sendWhiteCommand(Lamp lamp, int cold, int warm) {
        ...
        IotProtos.DirectSettings ds = IotProtos.DirectSettings.newBuilder()
                .setRed(0) // Resetujemy RGB w trybie cyrkadialnym (lub zachowujemy, zależy od logiki)
                .setGreen(0)
                .setBlue(0)
                .setColdWhite(cold)
                .setWarmWhite(warm)
